resource "null_resource" "first_control_plane" {
  ...
  depends_on = [
    ...
    null_resource.all_control_planes
  ]
}
...
# This is where all the setup of Kubernetes components happens
resource "null_resource" "kustomization" {
  ...
  provisioner "remote-exec" {
    inline = concat([
      ...
      # Check that /var/post_install exists and is a directory containing kustomization files
      <<-EOT
      ls -lad /var/post_install /var/post_install/rancher.yaml /var/post_install/kustomization.yaml
      EOT
      ]
      ,
